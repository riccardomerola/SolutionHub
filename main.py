...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.button_save = ctk.CTkButton(master=self.left_frame, text="Salva in database 💾", font=("Roboto", 15), fg_color="green", hover_color="#218838")
        ...
        values = [["ID", "Componente", "Problema", "Soluzione", "Documento"]]
        ...
        self.value_table.grid(row=1, column=0, padx=5, pady=5, sticky="ew")
    ...
    def handle_table_click(self, event):
        ...
        for i in range(1, self.value_table.rows):
    ...
    def format_data(self, rows):
        header = ["ID", "Componente", "Problema", "Soluzione", "Documento"]
        formatted_data = []
    ...
    def load_data(self):
        ...
        values = self.format_data(raw_rows)

        self.value_table.values = values
        self.value_table.update_values(values)

        try:
            self.value_table.edit_row(0, font=("Roboto", 18, "bold"))
        except:
            print("Errore nel font dell'header, applico stile base")
            self.value_table.edit_row(0)
...
class DetailWindow(ctk.CTkToplevel):
    def __init__(self, master, data):
        ...
        self.win_frame.grid_columnconfigure(0, weight=1)
        ...
        self.button_close_win.grid(row=5, column=0, padx=10, pady=10, sticky="sew")
